...
const roomColors = { 
  living: 0x4f46e5,   // Indigo-600
  bedroom1: 0x0891b2, // Cyan-600
  bedroom2: 0x059669, // Emerald-600
  kitchen: 0xd97706,  // Amber-600
  bathroom: 0xdb2777, // Pink-600
  corridor: 0x4b5563  // Gray-600
};
  ...
  setTimeout(() => {
    ...
    for (let i = 0; i < w * h; i++) {
      ...
      if (visited[i] === 0 && isWallInk && data[i * 4 + 3] > 50) {
        ...
        // Threshold for ink: standard plans look for dark lines. 
        // We added a "blob size" check to ignore small noise like door swings or text.
        if (blob.length >= 45) blob.forEach(px => grid[px] = GRID_WALL); // Increased min blob size to 45
        else blob.forEach(px => grid[px] = GRID_OUTSIDE);
      }
    }

    // --- 1.25 FILTER DOOR SWINGS (Experimental) ---
    // If a wall line is too thin and curved, it's likely a door swing.
    // We can identify these by looking for wall pixels that don't have many neighbors in a block.
    // For now, increasing the blob threshold above does most of the heavy lifting.

    // --- 1.5 BRIDGE DOOR & WINDOW GAPS ---
    // Detect gaps between wall pixels and mark them as doors (-10) or windows (-20)
    // Increased DOOR_GAP_LIMIT to catch larger door openings (45px)
    const DOOR_GAP_LIMIT = 45; 
    for (let y = 5; y < h - 5; y++) {
      for (let x = 5; x < w - 5; x++) {
        const i = y * w + x;
        if (grid[i] === GRID_EMPTY) {
          ...
          if (grid[i - 1] === GRID_WALL) {
            ...
            while (x + gap < w && grid[y * w + (x + gap)] === GRID_EMPTY && gap < DOOR_GAP_LIMIT) {
               // Check if there's any "blue-ish" or "cyan" tint here for windows
               let idx = (y * w + (x + gap)) * 4;
               if (data[idx + 2] > 200 && data[idx] < 150) isWindowColor = true; 
               gap++;
            }
            if (gap < DOOR_GAP_LIMIT && grid[y * w + (x + gap)] === GRID_WALL) {
              const marker = isWindowColor ? -20 : -10;
              for (let k = 0; k < gap; k++) grid[y * w + (x + k)] = marker;
            }
          }
          // Check vertical gap
          if (grid[i - w] === GRID_WALL) {
            let gap = 0;
            let isWindowColor = false;
            while (y + gap < h && grid[(y + gap) * w + x] === GRID_EMPTY && gap < DOOR_GAP_LIMIT) {
               let idx = ((y + gap) * w + x) * 4;
               if (data[idx + 2] > 200 && data[idx] < 150) isWindowColor = true;
               gap++;
            }
            if (gap < DOOR_GAP_LIMIT && grid[(y + gap) * w + x] === GRID_WALL) {
              const marker = isWindowColor ? -20 : -10;
              for (let k = 0; k < gap; k++) grid[(y + k) * w + x] = marker;
            }
          }
        }
      }
    ...
    function greedyMesh(targetValue) {
      ...
      for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
          ...
          // Rigid matching: only group pixels of the EXACT same value
          if (val === targetValue && meshVisited[y * w + x] === 0) {
            let width = 0;
            while (x + width < w && grid[y * w + (x + width)] === targetValue && meshVisited[y * w + (x + width)] === 0) {
              width++;
            }
            let height = 1;
            let canExpand = true;
            while (y + height < h && canExpand) {
              for (let i = 0; i < width; i++) {
                if (grid[(y + height) * w + (x + i)] !== targetValue || meshVisited[(y + height) * w + (x + i)] === 1) {
                  canExpand = false;
                  break;
                }
              }
            ...
            if (width > 0 && height > 0) {
              blocks.push({ 
                x, y, width, height, 
                isDoor: targetValue === -10, 
                isWindow: targetValue === -20 
              });
            }
          }
        }
    ...
    // Run meshing for each type separately to ensure clean separation
    const wallBlocks = [
      ...greedyMesh(GRID_WALL),
      ...greedyMesh(-10), // Doors
      ...greedyMesh(-20)  // Windows
    ];
...
function drawSampleOnCanvas(c, ctx) {
  ...
  // Internal walls
  ctx.beginPath();
  ctx.moveTo(330, 50); ctx.lineTo(330, 450);
  ctx.moveTo(50, 250); ctx.lineTo(330, 250);
  ctx.moveTo(330, 250); ctx.lineTo(510, 250);
  ctx.stroke();

  // Draw Doors (White boxes to break walls)
  ctx.fillStyle = '#ffffff';
  ctx.fillRect(320, 100, 20, 60); // Main interior door
  ctx.fillRect(320, 300, 20, 60); // Kitchen door
  ctx.fillRect(150, 240, 60, 20); // Bedroom door

  // Draw Windows (Cyan lines)
  ctx.strokeStyle = '#22d3ee';
  ctx.lineWidth = 10;
  ctx.beginPath();
  ctx.moveTo(650, 150); ctx.lineTo(650, 250); // Window 1
  ctx.moveTo(50, 150); ctx.lineTo(50, 250);   // Window 2
  ctx.stroke();

  ctx.fillStyle = '#000000'; ctx.font = '700 18px sans-serif';
  ctx.fillText('Living Room', 100, 150); ctx.fillText('Master Bed', 100, 360);
  ctx.fillText('Bedroom 2', 360, 150); ctx.fillText('Kitchen', 370, 360); ctx.fillText('Bath', 560, 150);
  
  // Add some dimensions for realism
  ctx.font = '12px sans-serif'; ctx.fillStyle = '#64748b';
  ctx.fillText('12\'0" x 14\'0"', 120, 170);
  ctx.fillText('10\'0" x 12\'0"', 380, 170);
}
...
function initThreeJS() {
  ...
  scene.background = new THREE.Color(0xf1f5f9); // Light background for contrast
...
function buildWalls() {
  ...
  // 1. Render Solid Wall Blocks with Door/Window Openings
  const EPSILON = 0.005; 
  wallsData.forEach(wall => {
    // Consistent light-gray walls for a clean architectural look
    let wColor = wall.type === 'outer' ? 0xffffff : 0xe2e8f0;
    ...
    if (wall.isDoor) {
      // Render Header only (top part of doorway)
      const doorHeaderH = heightM * 0.15; 
      const geo = new THREE.BoxGeometry(wall.widthUnits - EPSILON, doorHeaderH, wall.heightUnits - EPSILON);
      const doorMat = new THREE.MeshStandardMaterial({ color: 0x64748b, roughness: 1 });
      const mesh = new THREE.Mesh(geo, doorMat);
      mesh.position.set(wall.centerX, heightM - doorHeaderH / 2, wall.centerZ);
    ...
    } else if (wall.isWindow) {
      // Render Lintel (Top) and Sill (Bottom)
      const sillH = heightM * 0.3;
      const headerH = heightM * 0.2;
      const glassH = heightM * 0.5;

      // Sill
      const sGeo = new THREE.BoxGeometry(wall.widthUnits - EPSILON, sillH, wall.heightUnits - EPSILON);
      const sMesh = new THREE.Mesh(sGeo, mat);
      sMesh.position.set(wall.centerX, sillH / 2, wall.centerZ);
      scene.add(sMesh); wallMeshes.push(sMesh);

      // Header
      const hGeo = new THREE.BoxGeometry(wall.widthUnits - EPSILON, headerH, wall.heightUnits - EPSILON);
      const hMesh = new THREE.Mesh(hGeo, mat);
      hMesh.position.set(wall.centerX, heightM - headerH / 2, wall.centerZ);
      scene.add(hMesh); wallMeshes.push(hMesh);

      // Glass (Cyan Transparent)
      const gGeo = new THREE.BoxGeometry(wall.widthUnits * 0.8, glassH, wall.heightUnits * 0.8);
      const gMat = new THREE.MeshStandardMaterial({ color: 0x22d3ee, transparent: true, opacity: 0.4, metalness: 0.9, roughness: 0.1 });
      const gMesh = new THREE.Mesh(gGeo, gMat);
      gMesh.position.set(wall.centerX, sillH + glassH / 2, wall.centerZ);
      scene.add(gMesh);
      // We don't add glass to wallMeshes so it's not clickable for analysis (or we could)

      [sMesh, hMesh].forEach(m => {
        m.castShadow = true; m.receiveShadow = true;
        const edges = new THREE.EdgesGeometry(m.geometry);
        m.add(new THREE.LineSegments(edges, new THREE.LineBasicMaterial({ color: 0x000000 })));
        m.userData = { wallId: wall.id, wallData: wall, phase: wall.phase, ownsMaterial: false };
      });
    } else {
      // Regular Wall
...
// === BLOCKCHAIN VERIFICATION — Real Stellar Testnet Integration ===

// ──────────────────────────────────────────────────────────────────
// CONFIG — Deployed Soroban contract on Stellar Testnet
// ──────────────────────────────────────────────────────────────────
const STELLAR_CONFIG = {
  // Archlab audit contract deployed on Stellar Testnet
  contractId: 'CCDPGMFXENTALXB6LU7EPRGNZBOKVDSZFHSL62MPHQW5BXQPXRCRGDK',
  // Funded testnet keypair for submitting transactions
  secretKey:  'SBEDOGRRIGPTX7TL4IZITCTGMZAZWO4Z76YU7CSVMG7RKFINYUMVP6CJ',
...
async function verifyOnStellar() {
  ...
  document.querySelector('#bcLoading p').textContent = 'Hashing structural data...';

  try {
    // ── Step 1: SHA-256 hash of audit data ──────────────────────────
    ...
    // ── Step 2: Load Stellar account ─────────────────────────────────
    document.querySelector('#bcLoading p').textContent = 'Connecting to Stellar Testnet...';
    ...
    try {
      ...
      // If account not found (404), fund it via Friendbot
      if (e.response && e.response.status === 404) {
        document.querySelector('#bcLoading p').textContent = 'New account detected. Funding via Friendbot...';
        try {
          const fbResponse = await fetch(`https://friendbot.stellar.org/?addr=${pubKey}`);
          if (!fbResponse.ok) throw new Error('Friendbot funding failed');
          
          // Wait for ledger to close (funding takes a few seconds)
          document.querySelector('#bcLoading p').textContent = 'Waiting for Stellar Network to confirm funding...';
          await new Promise(r => setTimeout(r, 4000));
          
          account = await server.loadAccount(pubKey);
          document.querySelector('#bcLoading p').textContent = 'Account funded successfully!';
        } catch (fbErr) {
          throw new Error('Could not fund testnet account automatically. Please fund ' + pubKey + ' at friendbot.stellar.org');
        }
      } else {
        throw e;
      }
    ...
    // ── Step 3: Build Stellar transaction with audit hash in memo ────
    document.querySelector('#bcLoading p').textContent = 'Broadcasting to Stellar Testnet...';

    // Memo is limited to 28 bytes — we store the first 28 chars of hash
    // Full hash is stored in the transaction description (manage data op)
    const hashShort = hashHex.substring(0, 28); // 28-char prefix for memo

    const transaction = new StellarSdk.TransactionBuilder(account, {
      fee: StellarSdk.BASE_FEE,
      networkPassphrase: STELLAR_CONFIG.networkPassphrase,
    })
      // ManageData operation: stores "archlab_audit" → hash on-chain
      .addOperation(StellarSdk.Operation.manageData({
        name:  'archlab_audit',
        value: hashShort,
      }))
      .addMemo(StellarSdk.Memo.text('Archlab Audit Hash'))
      .setTimeout(30)
      .build();
    ...
    // ── Step 5: Show success with real tx hash ────────────────────────
    ...
    const r = document.getElementById('bcResult');
    r.style.display = 'block';
    r.innerHTML = `
      <h3>⛓️ Stellar Testnet — Verified ✅</h3>
      <p style="font-size:12px;color:var(--muted);margin:4px 0">Audit hash registered on-chain via Soroban ManageData</p>
      <div style="background:#0f1f2e;border-radius:8px;padding:10px 14px;margin:12px 0;text-align:left">
        <p style="font-size:11px;color:var(--muted);margin:0 0 2px">AUDIT SHA-256</p>
        <p class="bc-hash" style="font-size:11px;word-break:break-all;margin:0">${hashHex}</p>
      </div>
      <div style="background:#0f1f2e;border-radius:8px;padding:10px 14px;margin:8px 0;text-align:left">
        <p style="font-size:11px;color:var(--muted);margin:0 0 2px">TRANSACTION HASH</p>
        <p class="bc-hash" style="font-size:11px;word-break:break-all;margin:0">${txHash}</p>
      </div>
      <p style="font-size:13px;margin:6px 0">🕐 Block Time: ${blockTime}</p>
      <p style="font-size:13px;color:var(--muted)">🌐 Network: Stellar Testnet · Horizon API</p>
      <a href="${txLink}" target="_blank"
         style="display:inline-block;margin-top:10px;color:var(--accent);font-size:13px">
      ...
      <button class="btn-primary" style="margin-top:16px"
              onclick="document.getElementById('bcOverlay').classList.remove('show')">Close</button>`;

  } catch (err) {
    console.error('Stellar submission error:', err);
    document.getElementById('bcLoading').style.display = 'none';
    const r = document.getElementById('bcResult');
    r.style.display = 'block';
    r.innerHTML = `
      <h3>⚠️ Blockchain Submission Failed</h3>
      <p style="font-size:13px;color:var(--muted);margin:8px 0">${err.message || 'Network error — check console'}</p>
      <p style="font-size:12px;color:var(--muted)">Tip: Make sure the testnet account is funded at friendbot.stellar.org</p>
      <button class="btn-primary" style="margin-top:16px"
              onclick="document.getElementById('bcOverlay').classList.remove('show')">Close</button>`;
  }
}
...
async function downloadPDF() {
  ...
  const _hBuf  = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(dataToHash));
  const mockHash = Array.from(new Uint8Array(_hBuf)).map(b => b.toString(16).padStart(2,'0')).join('');
